...
Choose **Primary metadata source** and an optional **Fallback source** independently. All four providers are available; None disables fallback and selecting the same provider twice is prevented. Fallback runs after an error, no match, incomplete metadata, or a low text-match score. One complete source record is retained rather than combining fields from different releases. AI/Discogs comparisons prefer the more complete album/genre/year/label result, retaining the primary on ties. AI in either slot requires cost confirmation.

### Page overrides
Use the panel to the right of metadata review to enter shared values, then enable **Use page overrides** on individual tracks or select all on the page. Blank fields retain individual choices; unchecking restores those choices. Genre overrides also become the required main-genre tag. Custom tags are comma-separated and additive: existing tags are retained and duplicate labels are not added, even when the individual track was set to overwrite tags. Overrides reset affected tracks to pending review and still require Accept and Apply. Version-protected titles remain protected. Applied tracks keep their applied override values when the panel changes.

Page custom tags default to additive mode. Enable **Overwrite existing tags** to replace checked tracks’ custom tags with the override pills and final main genre; other per-track tag selections are excluded. An empty pill list retains only the main genre, or clears all tags if the final genre is empty. The mode badge and apply confirmation indicate replacement.
